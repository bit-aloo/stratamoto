...
fn an_unanswered_setup_connection_fails() {
    assert!(check(&pool(), &setup(Protocol::Mining, 2, 2, 0), &SetupResponse::Silence).is_err());
}

#[test]
fn an_answer_that_is_neither_success_nor_error_fails() {
    let response = SetupResponse::Unexpected { message_type: 0x10 };
    assert!(check(&pool(), &setup(Protocol::Mining, 2, 2, 0), &response).is_err());
}
...
fn any_error_code_is_accepted() {
    ...
    assert!(check(&pool(), &setup(Protocol::Mining, 2, 2, 0), &response).is_ok());
}
...
fn a_version_the_client_did_not_propose_fails() {
    ...
    assert!(check(&pool(), &setup(Protocol::Mining, 3, 2, 0), &response).is_err());
}
...
fn fixed_version_against_a_client_requiring_version_rolling_fails() {
    ...
    assert!(check(&pool(), &spec, &response).is_err());
}
...
fn accepting_a_subprotocol_the_role_does_not_serve_fails() {
    ...
    assert!(check(&pool(), &setup(Protocol::TemplateDistribution, 2, 2, 0), &response).is_err());
}
...
fn the_real_pool_contract_passes() {
    ...
    assert_eq!(check(&pool(), &spec, &response), Ok(()));
}
