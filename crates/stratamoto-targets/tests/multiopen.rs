...
fn program(opens: usize, reserved_frame_after_first: bool) -> Program {
    ...
    for i in 0..opens {
        let request = one(builder
            .append_op(Operation::LoadRequestId(100 + i as u32), &[])
            .unwrap());
        let identity = one(builder
            .append_op(Operation::LoadStr(String::new()), &[])
            .unwrap());
        let hashrate = one(builder
            .append_op(Operation::LoadHashrate(1_000_000_000f32.to_bits()), &[])
            .unwrap());
        let target = one(builder
        ...
        if i == 0 && reserved_frame_after_first {
            let bytes = one(builder
                .append_op(Operation::LoadBytes(vec![0u8; 14]), &[])
                .unwrap());
            builder
                .append_op(
...
fn opens_are_answered_until_the_client_sends_something_unhandled() {
    ...
    for (i, outcome) in answered.iter().enumerate() {
        ...
        assert_eq!(
            *request_id,
            100 + i as u32,
            "open {i} answered the wrong request"
        );
        assert!(job_id.is_some(), "open {i} came with no job to mine");
        assert!(
            identifiers.insert(*channel_id),
            "channel id {channel_id} was handed out twice"
        );
    }
